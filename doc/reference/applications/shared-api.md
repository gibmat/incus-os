...
On rare occasions, it might be necessary to temporarily rollback an application to an earlier version. This can be accomplished via the `set-version` command

```
incus admin os application set-version <name> -d '{"version":"<version>"}'
```

where `<version>` is one of the available application versions as reported by the `application show` command.

If no version is specified, IncusOS will attempt to rollback the application to the prior available version. `set-version` can also be used to update an application's version.
