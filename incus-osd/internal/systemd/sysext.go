...
func bestApplicationVersion(appName string, version *availableApplicationVersions, osInfo *state.OS) string {
	...
	if osInfo.PriorBootRelease == "" || osInfo.PriorBootRelease == osInfo.RunningRelease {
		...
		if sysextImageExists(appName, version.currentOSVersion) {
	...
	if strings.Compare(osInfo.PriorBootRelease, osInfo.RunningRelease) > 0 {
...
func sysextImageExists(name string, version string) bool {
	_, err := os.Stat(filepath.Join(LocalExtensionsPath, name+"_"+version+".raw"))
