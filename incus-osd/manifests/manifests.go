...
func Read(root string, manifests []string) (map[string]IncusOSManifest, error) {
	...
	for _, manifestName := range manifests {
		...
		// Every mkosi manifest will include the version in its filename, except for the base manifest.
		// Use this and the fact the base is always first to automatically extract the correct version string.
		versionString := ""
		if manifestName != "base" {
			versionString = "_" + ret["base"].Config.Version
		}

		// By default, assume we're reading a manifest generated by mkosi.
		file := filepath.Join(root, manifestName+versionString+".manifest")

		// If the file doesn't exist with the ".manifest" extension, try ".json".
		_, err := os.Stat(file)
		if err != nil {
			file = filepath.Join(root, manifestName+versionString+".json")
		}
...
func Write(root string, manifests map[string]IncusOSManifest) error {
	...
	for manifest := range manifests {
		...
		filename := manifest + "_" + manifests[manifest].Config.Version + ".manifest.json"
		if manifest == "base" {
